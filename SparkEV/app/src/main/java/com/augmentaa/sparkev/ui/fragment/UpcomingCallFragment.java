...
public class UpcomingCallFragment extends Fragment {
    ListView listView;
    View view;
    List<Data> list;
...
//            getCallHistory();
  ...
  /*  private void getCallHistory() {
        ...
        this.apiInterface.getCallHistory(hashMap1,new CallRequest("O", Pref.getIntValue(Pref.TYPE.USER_ID.toString(), 0), Pref.getValue(Pref.TYPE.MOBILE.toString(), null))).enqueue(new Callback<Question>() {
                            ...
                            if (loginResponse.data.size() > 0) {
                                for (int i = 0; i < loginResponse.data.size(); i++) {
                                    if (loginResponse.data.get(i).status.equalsIgnoreCase("O")) {
                                        ...
                                        UpcomingCallAdapter adapter = new UpcomingCallAdapter(getActivity(), list);
